...
llm = "mistral"
df1 = pd.read_csv(
    "https://huggingface.co/datasets/Kazchoko/codeinsights_llm_simulation/resolve/main/scenario_results/{llm}/{llm}_scenario2.csv"
)
df2 = pd.read_csv(
    "https://huggingface.co/datasets/Kazchoko/codeinsights_llm_simulation/resolve/main/scenario_results/{llm}/{llm}_scenario3.csv"
)
df3 = pd.read_csv(
    "https://huggingface.co/datasets/Kazchoko/codeinsights_llm_simulation/resolve/main/scenario_results/{llm}/{llm}_scenario4.csv"
)
df = pd.concat([df1, df2, df3], axis=0, ignore_index=True)  # stack rows
...
def load_ability(path, model_name):
...
# 2) load each one
claude_df = load_ability(
    "CodeInsights LLM Abilities/claude-3-5_student_ability.csv", "claude"
)
mistral_df = load_ability(
    "CodeInsights LLM Abilities/mistral_student_ability.csv", "mistral"
)
gemini_df = load_ability(
    "CodeInsights LLM Abilities/gemini-2.5-pro_student_ability.csv", "gemini"
)
gpt_df = load_ability("CodeInsights LLM Abilities/gpt-4o_student_ability.csv", "gpt4o")
llama_df = load_ability(
    "CodeInsights LLM Abilities/llama3.1-8b_student_ability.csv", "llama3.1"
)
gemma_df = load_ability(
    "CodeInsights LLM Abilities/gemma-3-27b-it_student_ability.csv", "gemma"
)
real_df = load_ability("CodeInsights LLM Abilities/real_student_ability.csv", "real")
merged = reduce(
...
ability_cols = [
    c for c in merged.columns if c.startswith("ability_") and c != "ability_real"
]
...
def load_difficulty(path, model_name):
    # only read the columns we need
...
# 2) load each one
claude_difficulty = load_difficulty(
    "CodeInsights_LLM_Difficulties/mistral_item_difficulty.csv", "claude"
)
mistral_difficulty = load_difficulty(
    "CodeInsights_LLM_Difficulties/mistral_item_difficulty.csv", "mistral"
)
gemini_difficulty = load_difficulty(
    "CodeInsights_LLM_Difficulties/gemini-2.5-pro_item_difficulty.csv", "gemini"
)
gpt_difficulty = load_difficulty(
    "CodeInsights_LLM_Difficulties/gpt-4o_item_difficulty.csv", "gpt4o"
)
llama_difficulty = load_difficulty(
    "CodeInsights_LLM_Difficulties/llama3.1-8b_item_difficulty.csv", "llama3.1"
)
gemma_difficulty = load_difficulty(
    "CodeInsights_LLM_Difficulties/gemma-3-27b-it_item_difficulty.csv", "gemma"
)
real_difficulty = load_difficulty(
    "CodeInsights_LLM_Difficulties/real_item_difficulty.csv", "real"
)
...
difficulty_cols = [
    c
    for c in merged_difficulty.columns
