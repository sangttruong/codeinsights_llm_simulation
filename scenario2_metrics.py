...
scenario2_student_df['tests'] = scenario2_student_df['question_unittests'].apply(parse_unittests)
result = { str(int(qid)): tests for qid, tests in zip(scenario2_student_df['question_id'], scenario2_student_df['tests']) }
jsonfile = json.dumps(result, indent=2)
scenario2_testcase_json = json.loads(jsonfile)

#Run LLM generated codes and make result dataframe
scenario2_results = []

for i in range(len(scenario2_df)):
    test_result = extract_student_code(scenario2_df.iloc[i]["text"])
    id_val = int(scenario2_df.iloc[i]["question_id"])
    student_id = scenario2_df.iloc[i]["student_id"]
    sub_question = question[question["question_id"] == id_val]

    # format testcases
    try:
        formatted_testcases, std_inputs = format_testcases(scenario2_testcase_json[str(id_val)])
...
scenario2_result_df = pd.DataFrame(scenario2_results)
scenario2_result_df["correctness"] = pd.Series(dtype="Int64")

# Where stdout == expected_output → 1, else → 0
scenario2_result_df["stdout"] = scenario2_result_df["stdout"].apply(
    lambda x: x.strip() if isinstance(x, str) else x
)
matches = scenario2_result_df["stdout"] == scenario2_result_df["expected_output"]
scenario2_result_df.loc[matches, "correctness"] = 1
scenario2_result_df.loc[~matches, "correctness"]     = 0

# If test_case_id is missing, reset correctness to <NA>
mask_missing = scenario2_result_df["test_case_id"].isna()
scenario2_result_df.loc[mask_missing, "correctness"] = pd.NA
scenario2_result_df['question_id'] = scenario2_result_df['question_id'] \
    .astype(str)
scenario2_result_df['test_case_id'] = scenario2_result_df['test_case_id'] \
    .astype(str)
scenario2_result_df = scenario2_result_df.rename(columns={
...
scenario2_student_df['num_unittest'] = scenario2_student_df['question_unittests'] \
    .astype(str) \
    .str.count(r'Unittest\s+\d+:')
scenario2_student_df['pass_str'] = scenario2_student_df['pass'].astype(str).str.split('.', n=1).str[0]
# 2) pad on the right with “0” to match num_unittest
scenario2_student_df['pass_padded'] = scenario2_student_df.apply(
    lambda r: r['pass_str'].ljust(int(r['num_unittest']), '0'),
    axis=1
)
# 3) explode into one digit per row
df_exploded = (
    scenario2_student_df
    .assign(pass_list = scenario2_student_df['pass_padded'].apply(lambda s: [int(ch) for ch in s]))
    .explode('pass_list')
...
mask = (
    scenario2_merged_df['LLM_correctness'].isin([0, 1]) &
    scenario2_merged_df['real_student_correctness'].isin([0, 1])
)
...
student_response_subset = scenario2_student_df[["student_id", "question_id", "response"]]
student_response_subset = student_response_subset.rename(columns={
     "response": "student_response"
 })
scenario2_df = scenario2_df.rename(columns={
    "text": "LLM_response"
})
scenario2_response_full = scenario2_df.merge(
    student_response_subset,
    on=["student_id", "question_id"],
    how="inner"
)
calculator = CodeSimilarityCalculator()
result_df = calculator.process_dataframe(scenario2_response_full)
